...
func TestFarmClient_Claim(t *testing.T) {
	...
	res, err := mockCli.Farm().Claim(fromInfo, passWd, "test-pool-name", memo, accInfo.GetAccountNumber(),
		accInfo.GetSequence())
	require.NoError(t, err)
	require.Equal(t, uint32(0), res.Code)
	...
	_, err = mockCli.Farm().Claim(fromInfo, "", "test-pool-name", memo, accInfo.GetAccountNumber(),
		accInfo.GetSequence())
	require.Error(t, err)
	...
	mockCli.EXPECT().BuildAndBroadcast(
		fromInfo.GetName(), passWd, memo, gomock.AssignableToTypeOf([]sdk.Msg{}), accInfo.GetAccountNumber(), accInfo.GetSequence()).
		Return(sdk.TxResponse{}, errors.New("default error"))
	_, err = mockCli.Farm().Claim(fromInfo, passWd, "test-pool-name", memo, accInfo.GetAccountNumber(),
		accInfo.GetSequence())
	require.Error(t, err)
}
